...
\begin{table}[H]
  ...
  \normalsize
\end{table}
\footnotetext{Collecting period inferred from Figure \ref{fig:ub_cwn_temporal_evol}.}
...
%
One important example of an individualist collector in the herbarium is `\textit{leite,alta}', with a total of $2,757$ records, none of which recorded as made collaboratively~(inset in Figure~\ref{fig:ub_team_sizes}).  % of records from all individualist collectors
...
\begin{figure}[h!]
    ...
    \caption[Coworking groups in the UB CWN.]{ Coworking groups in a subset of the UB CWN. A total of $30$ distinct communities are differentiated by color. Nodes sizes \luiz{Node sizes?} are proportional to the total number of records of collectors in the dataset. The strength of the ties between each pair of collectors is represented as links thickness. Graph layout was computed using the \textit{ForceAtlas2} algorithm \cite{Jacomy2014}. For a better visualization experience refer to the interactive version of the graph (\url{https://lncc-netsci.github.io/pedrocs/networks/ub_cwn}).}
