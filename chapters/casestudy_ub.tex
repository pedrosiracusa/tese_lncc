...
Some of these communities correspond to communities of interest, from the SCN model.
%
The bryophytes research group, led by `\textit{camara,peas}' is also cohesive in terms of field expeditions.
The group of collectors interested on herbaceous plants, led by `\textit{munhoz,cbr}'.

Others include nodes that form communities in one but not in the other.
%
\textit{Chaves,da} and \textit{Bringel,jba} are tightly connected in the common interests network ( in \textit{grp0}) but are distant in the CWN, having no connections.
%
\textit{Faria,J.E.Q} and \textit{Zanatta} are relatively strong collaborators although they show distinct taxonomic interests.

There are also communities that are formed due to the activities of their collectors being temporally spaced from others in the network (as the green one, around `\textit{irwin,hs}'.
