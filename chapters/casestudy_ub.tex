...
Table \ref{table:ub_collectors_florescer} lists some historically relevant collectors who have intensively contributed to the herbarium, according to the \textit{Florescer project}. 
\begin{table}[H]
...
However, most fields are either entirely composed of missing values or are irrelevant for our application, and therefore we selectively loaded only a subset of them, listed below.
Almost all fields (except for \textit{issue}) follow \textit{Darwin Core} terms standards\footnote{\url{http://rs.tdwg.org/dwc/terms}}:
% Terms definitions
...
After initially loading all $185311$ records (with the associated fields from the subset above) into computer memory for performing the data exploratory analysis, we filtered out records with missing values for either field \textit{recordedBy} or \textit{scientificName}, as these are critical for the construction of our network models.
In addition, as we intended to build our models at the taxonomic resolution of \textit{species}, we filtered out $17.77\%$ of the records, for which the taxonomic resolution was below that rank (Table \ref{table:dset_taxonomicres_counts}).
Taxa at higher resolutions were kept as the taxonomic identity at higher ranks of a taxon can be directly obtained.
The resulting dataset had a total of $x$ records. 
...
% 
Not every record has been identified up to the taxonomic resolution of \textit{species}, as shown in Table \ref{table:dset_taxonomicres_counts}. 
As taxons relate to each other on a tree-like hierarchical structure (each child taxon has exactly one parent, but a parent taxon can have one or more children), those at higher taxonomic ranks can be directly derived from more restrictive ones, but not the inverse. 
...
\begin{table}[t]
  \caption{Number of distinct taxa at each taxonomic rank in the dataset. For each rank a list with its top-5 most recorded taxa is included with their respective counts.}
\begin{center}
  \begin{tabular}{c c c c c}
    & num of taxa & top-5 taxa & num of records & \% of records \\
   \hline
    kingdom & 5 &
