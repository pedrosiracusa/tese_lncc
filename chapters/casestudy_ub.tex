...
Yet, some collectors in the herbarium have recorded much more species than the average, as it is the case of \textit{Howard S. Irwin} (\textit{irwin,hs}), with a total of $4535$ records of distinct species; or \textit{Carolyn E. B. Proença} (\textit{proenca,ceb}), with $1888$ distinct species. These are not only very experienced collectors but ones who also display a generalist recording behavior, meaning they're inclined towards recording the highest species diversity as possible. 
...
\begin{table}[t]
\caption{ Some degree metrics for the UB SCN model. For each nodes set the total number of nodes, average degree $\langle k \rangle$, top-10 highest-degree nodes and their respective degrees $k$ and normalized degree centralities $k^*$ are listed.}
\begin{center}
...
In our case data shows that collectors hubs tend to be more central than species nodes.
% Cite "Analyzing affiliation networks" from Borgatti, in Sage book of social networks analytics
