...
\begin{sidewaystable}
\begin{table}[H]
  \caption{Taxa composition for each community as illustrated in Figure \ref{fig:ub_scn_family_projCol_communities}. The total number of distinct taxa, together with the percentages of the top-3 most prevalent ones in each community are given.}
  \begin{center}
...
These are generalist collectors, having recorded $104$ from a total of $161$ distinct families, being families \textit{Fabaceae} ($18.8\%$ of the group interest), \textit{Rubiaceae} ($10.1\%$) and \textit{Myrtaceae} ($8.4\%$) the most prevalent ones. 
%
The giant component also includes other smaller communities which are more specialized, such as
\textit{grp $4$}, composed by collectors showing interests towards family \textit{Myrtaceae} ($62.0\%$); 
\textit{grp $2$} towards family \textit{Fabaceae} ($58.8\%$);
\textit{grp $3$} including herbaceous plants collectors, mainly from families \textit{Poaceae} ($40.4\%$), \textit{Cyperaceae} ($12.5\%$) and \textit{Asteraceae} ($7.5\%$); 
\textit{grp $0$} including majoritarily \textit{Asteraceae} ($72.7\%$) collectors;
\textit{grp $1$} mostly composed by collectors interested in families \textit{Cyperaceae} ($50.4\%$), \textit{Fabaceae} ($9.9\%$) and \textit{Oxalidaceae} ($7.2\%$) and
\textit{grp $5$} composed by \textit{Rubiaceae} ($82.7\%$) collectors. 
...
The second biggest connected component (\textit{grp{$7$}}) includes a more generalist part of the bryophytes collectors within polygon $(i)$ in Figure \ref{fig:ub_scn_agg_family_general}, where the most prevalent family is \textit{Sematophyllaceae} ($26.8\%$), followed by \textit{Hypnaceae} ($16.0\%$) and \textit{Dicranaceae} ($13.8\%$). 
...
% TODO [connection] Now what if we want to have a better notion of taxonomic groups that best split collectors interests ? 
...
Family \textit{Fabaceae}, for instance, is relatively more prevalent in community $2$ (with $58.8\%$ of the records in that community), although it is also included as a top-3 taxon in communities $0$, $1$, $4$, $5$, $6$, $11$, $17$, $18$ and $19$.
...
A better approach for retrieving such information is described below, and consists of changing the projection perspective, focusing on the species set.
...
\begin{figure}[!ht]
    ...
    \caption{ Communities in the species projection of the family-aggregated SCN in figure \ref{fig:ub_scn_agg_family_general}, in which edges with weights lower than $20$ were removed as well as isolated nodes. In order to improve visualization, communities with scores lower than $600$ were also omitted from the figure. Colors are used to distinguish communities.}
    \label{fig:ub_scn_family_projSp_communities}
\end{figure}
% The number of nodes and edges of the family-aggregated network is lower than the collectors projection
...
As the process produces many islands, we use the same relevance score from the collectors projection for filtering out non-relevant components, though now using a threshold value of $500$.
...
The species projection has a total of $15$ communities, three of which belong to the giant component. Every other component in the graph 



% Small nodes are strongly correlated -> The same groups of collectors recording distinct families

% Some very prevalent families are connected to many less prevalent ones
