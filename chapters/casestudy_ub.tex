...
%
In order to detect such groups we've applied the same algorithm for community detection \cite{Blondel2008} as we did for the SCN projections.
%
...
Some of the communities we found in the CWN are composed by collectors who also compose tight communities of interests from the SCN projection (Figure \ref{fig:ub_scn_family_projCol_communities}).
%
One such example is the bryophytes research group, which includes \textit{Paulo Câmara} (\textit{camara,peas}), \textit{Micheline Carvalho Silva} (\textit{carvalhosilva,m}) and \textit{Abel Soares} (\textit{soares,aer}).
Collectors from this coworking group (colored in purple and located in the northeast region of the giant component in Figure \ref{fig:ub_cwn_communities}) are not only more inclined towards collaborating with each other in field, but are also interested on recording the specific taxonomic group of bryophytes.
%
Another example is the coworking group of herbaceous plants collectors (colored in pink), including \textit{Cássia Munhoz} (\textit{munhoz,cbr}), \textit{Aryanne Amaral} (\textit{amaral,ag}), \textit{Thiago de Roure Bandeira Mello} (\textit{mello,trb}) and \textit{Chesterton Eugênio} (\textit{eugenio,cuo}).

% same coworking but distinct interests
Other coworking communities include collectors who happen to be strongly connected by collaborative ties in the CWN despite having distinct taxonomic interests, being weakly connected in the SCN projection.
One such case is that of \textit{Jair Faria} (\textit{faria,jeq}) and \textit{Maria Rosa Zanatta} (\textit{zanatta,mrv}), having recorded a total of $1524$ specimens together (comprising $64.5\%$ of all records by \textit{zanatta,mrv} and $32.5\%$ of all records by \textit{faria,jeq}).
Their species bags is substantially dissimilar, though, as `\textit{faria,jeq}' is more focused on family \textit{Myrtaceae} whereas `\textit{zanatta,mrv}' has a more generalist recording profile. 
They are included in distinct interest communities (\textit{grp4} and \textit{grp6}, Figure \ref{fig:ub_scn_family_projCol_communities}) but in the same coworking group (colored in light blue, Figure \ref{fig:ub_cwn_communities}).
% Case of heringer,ep and eiten,g

% same interest but distinct coworking
There are also cases of collectors who, despite being very similar in their taxonomic interests (typically strongly tied in the SCN projection), seldom or never collaborate in field, and are thus not included in the same coworking groups.
\textit{Chaves,da} and \textit{João Bringel} (\textit{bringel,jba}), for instance, are tightly connected in the common interests network ( in \textit{grp0}) but are distant in the CWN, having no connections.
% Maybe due to geographical or temporal spacing
%


% Temoral evolution of the CWN
Some communities reflect a temporal spacing on the recording activities of collectors.
As collectors careers have limited lifespans, they can only collaborate with contemporary collectors.
Therefore, the formation of communities in CWNs is temporally constrained. 
Figure \ref{fig:ub_cwn_temporal_evol} shows the temporal evolution of the network from Figure \ref{fig:ub_cwn_communities}.
\begin{figure}[h!]
  	\centering
