...
\textit{Florescer project} webpage \footnote{\label{footnote:florescer} \url{http://florescer.unb.br/janela4.html}}.
...
\begin{table}[H]
  ...
  \end{tabular}
  \\[1.5em]
  \hfill Source: Florescer Project webpage \footnotemark[\getrefnumber{footnote:florescer}]
  \end{center}
...
Instead, the main \textit{Myrtaceae} collector in the herbarium is \textit{Jair E. Faria} (\textit{faria,jeq}), who apparently has a preference towards this family ($\approx 31.0\%$ of all his records). \textit{Carolyn E. B. Proença} (\textit{proenca,ceb}) is another key collector of this family, although she is also interested, to the same extent, in families \textit{Fabaceae} and \textit{Asteraceae}. Moreover, the figure also makes it easy to detect collectors who exclusively (or almost exclusivelly) collect each family, as it is the case of \textit{Vanessa G. Staggmeier} (\textit{staggmeier,vg}) for \textit{Myrtaceae} and \textit{Regina C. Oliveira} (\textit{oliveira,rc}) for \textit{Poaceae}.
...
The core could certainly be further partitioned into communities by some algorithms, although we will not perform it here.
The largest one
