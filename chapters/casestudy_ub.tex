...
From network structure we can also identify collectors that are most relevant for the herbarium, and how they contribute for the representativeness of each taxonomic group in the collection.
...
\begin{table}[H]
  \caption{Historically important collectors for the UB herbarium.}
  \begin{center}
  \begin{tabular}{l l l}
...
The entire dataset was downloaded through the portal and loaded into memory in a personal computer for analysis.
%
Before building the network models we first performed a quick exploratory analysis of the UB dataset, for grasping a general overview on its taxonomic composition, the temporal and geographical distribution of the records, as well as the main issues associated to the dataset (section \ref{section:ub_exploration}).
...
\item [recordedBy.] Contains a list with the names of people or groups who have authored an occurrence record. 
...
Table \ref{table:taxa_counts} shows the number of distinct taxa at each rank and lists those with the highest numbers of records.
...
Before we could use the UB occurrences dataset for actually building the network models, we the submitted the tabular dataset to some data filtering and transformation routines.
...
In order to build the SCN at the taxonomic resolution of \textit{species} we only selected records at equal or lower ranks, for which the taxonomic identity could be resolved at the level of \textit{species}.
In other words, we used records at resolutions of \textit{form}, \textit{variety}, \textit{subspecies} and \textit{species}, corresponding to $82\%$ of the original records from the dataset (a total of $152379$ records, as shown in Table \ref{table:dset_taxonomicres_counts}).
...
The names atomization step consisted of iterating each record from the UB dataset and splitting the corresponding string containing collectors names, which is stored in the \textit{recordedBy} field. 
We splitted the strings on the semicolon character (` ; '), which is used as the delimiter for the majority of entries.
...
The process originates a list of strings for each record, containing the names of each collector associated to it.
A new field named \textit{recordedBy\_atomized} is created for holding the lists of names, and is later used to construct the network models.
...
Another community of algae collectors is the one formed by `\textit{leite,alta}', `\textit{grando,jv}' and `\textit{castelobranco,cw}' (community $ii$), which also happens to be the connected component $c_2$ itself.
...
By inspecting \ref{fig:ub_cwn_communities}, we verify that in fact all those collectors are in strategic positions.
