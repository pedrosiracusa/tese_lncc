...
%
We investigated the temporal evolution of communities in the UB CWN by rendering a sequence of frames covering the period from $1950$ to $2017$, each spanning a period of $7$ years (Figure \ref{fig:ub_cwn_temporal_evol}).
Frames aggregate and display collaborations (edges) occurring within the respective periods, and only collectors who have recorded at least one specimen within each period are represented. 
Although our proposed CWN model does not incorporate the temporal dimension in its structure, we were able to build the frames by querying the UB species occurrence dataset, based on which the network was constructed.

Three pioneer collectors groups, which we refer to the first generation of botanists (with most of their recording activities concentrated from $1958$ to $1981$), were historically remarkable for having largely contributed to the formation of the UB herbarium (frames $1-4$ in Figure \ref{fig:ub_cwn_temporal_evol}).
% First generation
% Blue group
The first group includes \textit{heringer,ep}, \textit{eiten,g} and \textit{eiten,lt}.
\textit{Ezechias Paulo Heringer} (\textit{heringer,ep}) is regarded as one of the founders of the UB herbarium, having led the first collection expeditions in which the first samples which became its basis were gathered \cite{Walter2001}.
His most intensive collecting activities date from $1950$ to $1981$, period in which he has collaborated with other important collectors such as \textit{Alberto Castellanos} (\textit{castellanos,a}) and \textit{George Eiten} (\textit{eiten,g}). 
\textit{Eiten} is also a historically relevant collector for the herbarium, having contributed with an extensive set of records of plants, mainly from the Cerrado biome \cite{Gomes2012}.
\textit{Eiten}'s known strong collaboration with first his first wife, \textit{eiten,lt}, is captured by our network model, as shown in frames $2$ and $3$ in Figure \ref{fig:ub_cwn_temporal_evol}.
\textit{Heringer}'s collaborative recordings with \textit{eiten,g} were concentrated within the period from $1974$ to $1981$, and took place at the states of Minas Gerais and Goiás.
...
Other important collectors, including \textit{harley,rm}, \textit{fonseca,sf} and \textit{onishi,e}, have later on joined the team around \textit{irwin,hs}, contributing to the expedition in the period from $1966$ to $1973$.
%
It is also noticeable the association between \textit{anderson,wr}, \textit{kirkbride-junior,jh} and \textit{stieber,mt}, in the time frame from $1966$ to $1973$.
...
The first generation we refer to is the one formed by collectors cited above (including \textit{irwin,hs}, \textit{anderson,wr} and \textit{harley,rm}), most of which have participated in the context of the Central Brazilian Highlands expeditions.
The second generation, which is described below, is composed by nodes in the more central region of the network, including \textit{proenca,ceb}, \textit{munhoz,cbr}.
...
% Purple group
The third group is smaller, composed of $6$ collectors including \textit{magalhaes,m} and \textit{belem,rp}, the latter a brazilian student who has mostly collected at the state of Bahia from $1950$ to $1973$.

% Second generation

The more central region of the network starts to grow around $1990$, with the remarkable ascension of `\textit{proenca,ceb}', `\textit{souza,mgm}',

\textit{ratter,ja} has an important role on bridging the collectors temporally (he has collaborated with both the teams of \textit{irwin} and \textit{proenca,ceb}.


\begin{figure}[h!]
  	\centering
    \includegraphics[width=\linewidth]{figures/casestudy_ub/cwn_temporal_evol.png}
    \caption{ Temporal evolution of the CWN shown in Figure \ref{fig:ub_cwn_communities}. Each frame corresponds to a period of $7$ years, and only collectors who have recorded at least one specimen in each period were drawn. Nodes sizes are proportional to the total number of records of each collector up to the end of each period; and the thickness of ties are proportional to how intensively collectors have collaborated during each period (using the hyperbolic weighting rule). }
    \label{fig:ub_cwn_temporal_evol}
\end{figure}

Three 
Three pioneer coworking groups:
The first collectors communities to be formed are those around \textit{Howard S. Irwin} (\textit{irwin,hs}); 
\textit{Ezechias Paulo Heringer} (\textit{heringer,ep});
\textit{Romeu P. Belém} (\textit{belem,rp});
(as the green one, around `\textit{irwin,hs}' (Figure \ref{fig:ub_cwn_temporal_evol}).
Mainly from $1950$ to $1973$.

\textit{Howard Irwin} is included in the same community of interest as `\textit{proenca,ceb}', as they are generalist collectors.
However, the CWN shows that they form distinct communities, due to the fact that their activities do not overlap very much temporally.
Their community is the link to the rest of the giant component through \textit{James Ratter} (\textit{ratter,ja}).



\begin{figure}[h!]
  	\centering
    \includegraphics[width=\linewidth]{figures/casestudy_ub/ub_cwn_munhoz_collabs.png}
    \caption{ Temporal evolution of collaborations by Dr. \textit{Cássia~B.R.~Munhoz} (`\textit{munhoz,cbr}', colored in blue), from $1990$ to $2013$. Each frame corresponds to a period of $3$ years, and only collectors who have collaborated at least once with `\textit{munhoz,cbr}' in each period were drawn (red nodes). Nodes sizes are proportional to the total number of records of each collector up to the end of each period; and the thickness of ties are proportional to how intensively collectors have collaborated during each period (using the hyperbolic weighting rule). }
    \label{fig:ub_cwn_munhoz_collabs}
\end{figure}
