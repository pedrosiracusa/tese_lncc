...
In order to build the SCN at the taxonomic resolution of \textit{species}, we next selected records for which the taxonomic identity could be resolved at that rank.
...
Therefore we used $82\%$ of the records in the dataset (a total of $152379$ records, as shown in Table \ref{table:dset_taxonmicres_counts}) for building the networks.
...
The second step consisted of iterating each row and splitting the string that stores collectors names, in the \textit{recordedBy} field. 
...
After the names atomization process, each string originates a list of strings and is stored in a new field named \textit{recordedBy_atomized}.
