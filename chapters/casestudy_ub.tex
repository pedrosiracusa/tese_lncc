...
% TODO: Add collecting team size distribution
...
The resulting network had a total of $6768$ collectors and $15344$ species nodes, with a total of $142647$ edges connecting nodes from each set. 
The maximum number of edges that could possibly exist in this network (in case every collector were connected to every species) is $|S_{col}| \times |S_{sp}| \approx 1.04 \times 10^{8}$.
The network density is calculated as the ratio between the number of actual nodes in the network and the maximum number of nodes if it was fully connected.
We get a network density of approximately $1.37 \times 10^{-3}$.


\paragraph*{The herbarium core.}
Most nodes in the network ($93.6\%$ of the collectors and $95.0\%$ of the species) are connected to a main giant component, which we will refer to as the herbarium core. 
\todo[inline]{Maybe we should better define the herbarium core somewhere. It should be defined in respect to a taxonomic rank and degree threshold...}
For a collector to be included in the herbarium core it must have collected at least one species in common with another collector who is also included in the core. The same reasoning applies to species nodes as well by observing the inverse relationship.
...
As opposed to random networks, in which extremely well connected nodes are unlikely to coexist with a large number of extremely poorly connected ones, the degree distribution observed for collectors in the UB SCN is better described by a power law $p(k) \sim k^{-\alpha}$, which are typically observed in many large real-world networks with the scale-free property \cite{Barabasi1999a}. The dashed line in figure \ref{fig:ub_scn_degree_dist}b is a power law fit for the $S_{sp}$ data, with $\alpha=1.52$. 
...
\subsection{The UB Collectors Coworking Netork}
...
\paragraph*{Who collaborates with whom?}
