...
For a slightly more comprehensive introduction we recommend the reader to refer to \textit{Newman}'s book on network science \cite{Newman2010b}.
...
%
Edges necessarily involve pairs of vertices, and can be either \textit{undirected} or \textit{directed}.
...
This representation is also compact enough to be adopted for graph storage in some situations.
%
...
\textbf{Adjacency matrices} are usually denoted $A$ and represent graph edges $(u,v)$ as elements $A_{u,v}$.
The adjacency matrix of undirected weighted graphs is always symmetric, as each edge $(u,v)$ is equivalent to $(v,u)$. 
Element $A_{u,v}>0 $\textit{iff}$(u,v)\in E$.
Non-zero elements store edges weight values.
%Besides the vertices-and-edges representations, graphs can also be represented as alternative data structures, which can be more appropriate depending on the application.
%Adjacency lists in computers
%Although graphs can be trivially represented as ... in computers, Adjacency matrices are appropriate in some contexts for mathematical operations.
%Undirected graphs have symmetric adjacency matrices, while not necessarily for directed graphs.
...
% Vertices basic metrics
A pair of vertices which are directly connected by an edge are said to be \textit{adjacent}.
The set of all vertices that are adjacent to a particular vertex compose its \textit{neighborhood}.
The number of edges connected to a vertex or, in other words, the size of its neighbors, is given by its \textit{degree}.
Denoted by $k$, 
$
k_i = \sum_j^{n} A_{i,j}
$
We can also obtain a global metric for the entire graph, the \textit{average degree} 
$
\langle k \rangle = k_i
$
The \textit{degree distribution}


% Nodes and edges attributes
Attributes can be associated to vertices and edges.
A graph may be weighted by an edge attribute. 

% Paths in directed/undirected graphs
...
Pairs of vertices are said to be \textit{connected} if at least one path involving them exists.

% Connected components
\textit{Connected components} are defined as subgraphs in which there is at least one path linking each pair of vertices.
...
%%%% Machanism: Preferential attachment, rich gets richer: Higher degree vertices (richer), lower degree vertices (poorer)
...
\textbf{Bipartite graphs}, also known as bigraphs or two-mode graphs, are a special class of graphs composed by two distinct sets of vertices $U$ and $V$, with the constraint that no vertices within the same set are allowed to be adjacent to each other. 
