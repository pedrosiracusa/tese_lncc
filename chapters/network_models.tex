...
Such relational systems are naturally represented as networks, where interactions are represented as pairwise connections between entities and assume particular semantics depending on the nature of the modeled phenomenon.
...
Following this approach they were able to generate models in which entities were separated from others by very few connections, even for very large networks.
...
A \textit{preferential attachment mechanism} was proposed by \textit{Barabási} as ruling network growth by preferentially connecting new entities to those in the network which already have many connections \cite{Albert2002}. 
...
We refer to networks with non-trivial topological features as \textit{complex networks}.
%% Examples of networks in biology, computer science...

Network models are suitable for structuring relational data, where entities are expected to interact with each other through connections (or links). 
...
Next we review some important definitions from graph theory.

\subsection{Concepts on Graph theory}
...
Objects are referred to as \textit{vertices} and the connections between them as \textit{edges}.
Graph structures provide a natural representation for networked systems for a couple of reasons.
First, they allow representing entities and their interactions in a structured way, a ``map of connections'' through which information flows.
Connections necessarily involve two entities and can be either symmetric, in which case information is allowed to flow in both directions; or asymmetric, if it necessarily flows from a source to a target entity.
Second, graph theory provides a bunch of of metrics and algorithms in literature for dealing with graphs and exploring their topologies.
Finally, the availability of a considerable amount of graph visualization tools and techniques potentializes obaining insights from its structure.

 
Graphs are formally defined as a pair $G=(S,E)$, where $s$ is the set of vertices that compose the graph; and $E$ is the set of all edges.
A pair of vertices directly connected by an edge are said to be \textit{adjacent}.
The set of all nodes that are adjacent to a particular node compose its \textit{neighborhood}.
Attributes can be associated to nodes and edges.
A graph may be weighted by an edge attribute. 
Edge weight represents the strenghtness of an edge.
If connections are symmetric the graph is \textit{undirected}; otherwise, \textit{directed}.
% <include figure>


Besides the nodes-and-edges representations, graphs can also be represented as alternative data structures, which can be more appropriate depending on the application.
Adjacency matrices are appropriate in some contexts for mathematical operations.
Undirected graphs have symmetric adjacency matrices.
If the graph is unweighted the matrix stores values $1$ in case an edge between two vertices exists and $0$ otherwise. 
As many real-world of networks are very sparse, adopting sparse matrices representations such as compressed row storage (CRS) might be useful.
...
For some applications it turns \luiz{out?} to be convenient to use a matrix representation of graphs. A matrix where ... is known as the graph's \textbf{adjacency matrix}. % where are zeros and ones?
...
We define bipartite graphs as 
$$
B = (U, V, E) \mbox{, }
$$
where $E$ is the set of edges between nodes from $U$ and $V$.
