...
Possible values for density are thus bound from $0$ to $1$, where $1$ is obtained for a \texti{complete graph} (or a \textit{clique}) and $0$ would be obtained for a graph with no edges.
...
%
The global clustering metric $c_{\delta}$, on the other hand, computes the fraction of triples in the graph that compose cliques (or triangles).In short, \equation[eq:globalclustering]{c_{\delta} = \frac{3 \times numOfTriangles}{numOfTriples}}
