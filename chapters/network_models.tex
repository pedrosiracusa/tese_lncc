...
In the context of this work we refer to the \textit{social networks framework} as a set of concepts, methods and algorithms which can be directly applied or slightly adapted for modeling and analyzing social networks in diverse and independent contexts and knowledge domains.
%
...
Here we describe some of them for giving the reader a more solid intuition on how they're structured before we introduce our own models.


% Affiliation networks
\textit{Affiliation networks} are a particular type of social networks in which individuals are associated to events, groups or institutions by being members or participating in them \cite{Borgatti2015}. 
Social ties are usually derived from such models by considering that individuals belonging to the same groups or attending the same events are more likely to become acquainted than those who are not co-affiliated to any of those. 
...
% Some commonly explored approaches:
...
Besides ecological interactions, other natural systems have also been modeled under the perspectives of network thinking. 
- Interactions of animals within their populations;

Modeling ecological interactions is very common.




%% Animal movement networks
%%% How tracking data is gathered?
Movement networks allow understanding how individuals or groups of individuals use geographical space in a structured way.
Spatial connectivity is modeled as networks of discrete locations, which are
Animal movement networks are structured by connecting discrete locations which are sequentially visited.    
Processes are composed of discrete movement steps in the network structure


\cite{Jacoby2016a}

Animal movement networks represent movement steps as discrete movement steps an animal can transition.
Animal movement is represented as a network of discrete movement steps, where locations are represented as nodes linked by transitions. 
Animal movement networks for structuring animal routes.
...
% Social networks composed by people / institutions
Few initiatives have applied social network analysis in the context of networks of people and institutions working with biodiversity.

%% Collaborative research in network ecology field (3.4)
Authors form collaborative clusters around subareas.
\cite{Borrett2014}

%% Network analysis in the biodiversity informatics landscape
