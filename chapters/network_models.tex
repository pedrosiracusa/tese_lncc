...
An adjacency matrix $A$ is defined as having non-zero entries $a_{i j}$\textit{ iff } \luiz{melhor {\em if and only if}, acredito que os biólogos não entenderão a abreviação} $(i,j)\in E$ which, in weighted graphs, correspond to the weight assigned to edge $(i,j)$.
