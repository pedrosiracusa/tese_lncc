...
In social networks terms, this relation represents the phenomenon that ``a friend of my friend is also my friend''. \art{'is' OU 'tends to' ???}\ped{Ao meu ver o certo é `is'... A relação de transitividade significa que se A-->B e B-->C então A-->C (ou seja, forma-se um clique). Concorda?}
...
We can similarly compute the \textit{weighted degree} for vertex $i$ as $ k_{w}^{(i)} = \sum_j a_{i j} $. \art{inverti os indices do k ponderado.}\ped{Ziviani aqui tive que reverter sua alteração... Estou usando a notação $k_w$ durante outras partes do texto para me referir ao `weighted degree', e o $i$ se refere a uma observação (um vértice particular).}
...
Mixing patterns describe the influence of characteristics of nodes on the formation of links in networks.
In case associations are more likely to be established between nodes which are similar on some characteristic, the pattern is known as \textit{assortative mixing} or, in social sciences, \textit{homophyly}.
