...
In social networks terms, this relation represents the phenomenon that ``a friend of my friend is also my friend''. \art{'is' OU 'tends to' ???}\ped{Ao meu ver o certo é `is'... A relação de transitividade significa que se A-->B e B-->C então A-->C (ou seja, forma-se um clique). Concorda?}\art{A definicao estah certa... entretanto, a afirmacao social ilustrativa eh que  faz pensar em nivel de transitividade (como seria para clustering), justamente pela afirmacao nao ser sempre verdadeira. Acho que pode ser mais seguro simplesmente omitir essa frase. Acho que resolveria, deixando a medicao de nivel de transitividade/clustering para o item anterior que estah OK.} \ped{Entendi seu ponto. Na verdade aqui eu quis dizer que a interpretação, no contexto de redes sociais, é que transitvidade entre tres individuos ocorre quando há esta relação. Vamos discutir e vou tentar refrasear.}
...
\art{Pedro, o usual é o (a) e (b) ficarem *abaixo* das subfiguras. Eh facil de mudar na figura acima?}
...
\art{A frase ficou bonita, mas alguem mais preciosista argumentaria que redes sociais vem antes de network sceince... talvez seja melhor refrasear}
%
...
\ped{Leiam este tópico com cuidado e vejam se o texto está impreciso ou confuso. A ideia da agregação é para ser bem simples.}\art{Fiz umas intervencoes de maquiagem no texto, mas achei que estah tudo claro e bem definido.}
...
\end{cases}
$$
In case every distinct path in the non-projected SCN only occurs once, then both simple and additive weighting rules lead to the same result. % TODO: Test this assumption 
...
\art{Additive weighting foi nome que voce deu???? Pq nao average weighting???}\ped{De fato talvez fosse um nome melhor. Essa pesagem inicialmente era uma soma ($u+v$). Mas causava um certo desbalanceamento quando dois coletores estavam ligados a uma espécie por arestas muito destoantes (por ex. $w(col_1, sp_1)=1000$ e $w(col_2, sp_1)=1$). Ao ligar $col_1$ e $col_2$, o peso somado era ainda muito alto ($1001$), ao passo que tirando a média ele cai para $500.5$. A média melhora um pouco a rede, mas ainda assim eu ia tentar encontrar uma função melhor, que considerasse melhor esse desbalanceamento. Acabei não voltando nessa questão, pq vi que a pesagem por similaridade de cossenos era mais útil... Deixei essa pesagem mais para mostrar a evolução do raciocínio. Depois vou ver onde mais usei este nome no texto e se vc achar q eh necessário posso trocar. De qualquer forma acho q no estudo de caso só usei a similaridade. }
\art{Eu deixaria entao 'average weighting' aqui, soh verificando se tem em outros lugares o termo para ajustar. Jah troquei abaixo o seguinte para 'similarity-based weighting'}

\paragraph*{Similarity-based weighting.}
%\art{Que tal `Similarity-based weighting'???? Acho melhor que o nome atual.}\ped{Melhor mesmo... vou trocar depois}
