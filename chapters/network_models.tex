...
A \textit{preferential attachment mechanism} was proposed by \textit{Barabási} as ruling network growth by preferentially connecting new nodes to those in the network that already have many connections \cite{Albert2002}. \art{O artigo original é de 1999... está citando o artigo certo?}
...
\art{Cuidado. Existem redes reais com caracteristicas mais proximas de redes aleatorias. Refraseei.}
...
A graph structure provides a natural representation for networked systems for a couple of reasons.
...
\art{No paragrafo abaixo troquei $G=(S,E)$ por $G=(V,E)$ por ser mais usual. Vou adaptando onde me parecer necessario, mas eh bom verificar se nao deixei nada para tras.}
...
\begin{figure}[h!]
    ...
    \art{eh edge list, nao edges list (precisa trocar na figura)}
    \art{Trocar $s_i$ por $v_i$ na Figura 2????}
    \label{fig:graphs}
  \end{figure}
