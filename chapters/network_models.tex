...
A \textit{preferential attachment mechanism} was proposed by \textit{Barabási} as ruling network growth by preferentially connecting new entities to those in the network which already have many connections. 
...
For a general intuition, \textbf{graphs} are mathematical structures composed by discrete objects holding pairwise connections to each other.
The most intuitive representation is the one in which objects are represented as vertices (or nodes), and links between them are edges.
Graphs are formally defined as a pair $G=(S,E)$, where $s$ is the set of nodes that compose the graph; and $E$ is the set of all edges.
Attributes can be associated to nodes and edges.

Graphs are a natural approach for representing networked systems:
1. Discrete entities can naturally be represented as vertices, and their interactions as edges.
2. There are a whole set of metrics and algorithms in literature for dealing with graphs and exploring their topologies.
3. There are a bunch of available tools for graph visualization, which potentializes drawing insights from their structures.

Besides the nodes-and-edges representations, graphs can also be represented in alternative data structures, which can be more appropriate depending on the application.
Adjacency matrix.


We review some important definitions from graph theory.
