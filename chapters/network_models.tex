...
\art{No paragrafo acima, mexi na pergunta para refletir a fracao de vizinhos (e nao numero absoluto como estava antes}

\art{Tambem no paragrafo acima, triplet nao estava explicado. Estendi o texto explicando isso (adaptacao do wikipedia :-) )}
...
In social networks terms, this relation represents the phenomenon that ``a friend of my friend is also my friend''. \art{'is' OU 'tends to' ???}
...
$,
where $a_{i j}$ are elements containing the weights of each edge $(i,j)$; and the function $bool(x)$ returns $1$ if $X$ is greater than zero; and returns $0$ otherwise.
We can similarly compute the \textit{weighted degree} for vertex $i$ as $ k_i^{w} = \sum_j a_{i j} $. \art{inverti os indices do k ponderado.}
...
The probability distribution of vertex degrees over a graph is known as its \textit{degree distribution}.
...
Mixing patterns describe the effects of node characteristics on influencing \luiz{Acho que pode tirar esse {\em influencing} pois no início da frase tem {\em the effects}} link formation in networks.
In case associations are more likely to establish \luiz{be established?} between nodes which are similar on some characteristic, the pattern is known as \textit{assortative mixing} or, in social sciences, \textit{homophyly}.
...
Degree correlation can be understood as a specific type of mixing pattern, evaluated on vertex degrees.
...
\section{Collectors Coworking Networks} \label{section:cwn}
